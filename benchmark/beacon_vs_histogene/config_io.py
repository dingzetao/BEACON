...
def load_benchmark_config(path: str | Path) -> dict:
    ...
    cfg["_prepared_dir"] = (
        root / cfg.get("prepared_dir", "benchmark/beacon_vs_histogene/prepared")
    ).resolve()
    h2 = cfg.get("hist2st_prepared_dir")
    cfg["_hist2st_prepared_dir"] = (root / h2).resolve() if h2 else None
    return cfg
