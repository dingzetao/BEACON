#!/usr/bin/env python3
"""
05 — Benchmark BEACON vs HisToGene-B against cell2location ground truth.
...
def main():
    parser = argparse.ArgumentParser(description="BEACON vs HisToGene-B benchmark")
    ...
    htg_dir = out_dir / "histogene_spots"
    ...
    for cell_type, target_cfg in cfg["targets"].items():
        ...
        for sample in cfg["samples"]:
            ...
            htg_csv = htg_dir / f"{paper_name}_{cell_type}.csv"
            ...
            if htg_csv.is_file():
                htg = pd.read_csv(htg_csv)
                htg["spot_id"] = htg["spot_id"].astype(str)
                merged_h = gt.merge(htg, on="spot_id", how="inner")
                if len(merged_h) >= 10:
                    method_runs.append(("HisToGene-B", merged_h, "histogene_pred"))
                else:
                    print(f"  [!] {paper_name}: only {len(merged_h)} matched spots (HisToGene-B)")
            else:
                print(f"  [!] missing {htg_csv}")
