#!/usr/bin/env python3
"""
05 — Benchmark BEACON vs Hist2ST-B against cell2location ground truth.

Prerequisites:
  01_prepare_visium.py
  02_train.py
  03_predict.py
  04_collect_beacon_spots.py
...
def main():
    parser = argparse.ArgumentParser(description="BEACON vs Hist2ST-B benchmark")
    ...
    hist_dir = out_dir / "hist2st_spots"
    ...
    for cell_type, target_cfg in cfg["targets"].items():
        ...
        for sample in cfg["samples"]:
            ...
            hist_csv = hist_dir / f"{paper_name}_{cell_type}.csv"
            ...
            if hist_csv.is_file():
                ...
                hist["spot_id"] = hist["spot_id"].astype(str)
                merged_h = gt.merge(hist, on="spot_id", how="inner")
                if len(merged_h) >= 10:
                    method_runs.append(("Hist2ST-B", merged_h, "hist2st_pred"))
                else:
                    print(f"  [!] {paper_name}: only {len(merged_h)} matched spots (Hist2ST-B)")
            else:
    ...
    if not per_sample_rows:
        raise RuntimeError("No samples evaluated. Run 01→04 first.")
    ...
    for _, row in val.sort_values(["cell_type", "method"]).iterrows():
        print(
            f"  {row['cell_type']:12s} {row['method']:10s} "
